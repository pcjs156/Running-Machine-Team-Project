# -*- encoding: utf-8 -*-
"""
멜론 연간 차트 순위 데이터를 파싱해 저장하는 프로그램 (1964 ~ 2020)
"""
...
if __name__ == '__main__':
    ...
    parsedFilenames = list(filter(lambda p: p.endswith('.txt'), os.listdir(os.path.join('.', 'yearly_melon_chartdata'))))
    parsedFilenames = list(filter(lambda p: not isEmptyFile(os.path.join('.', 'yearly_melon_chartdata', p)), parsedFilenames))

    for year in range(1964, 2020+1):
        # 'https://www.melon.com/chart/search/list.htm?chartType=YE&age=1960&year=1964&classCd=KPOP&moved=Y'
        url = f'https://www.melon.com/chart/search/list.htm?chartType=YE&age={getAge(year)}&year={year}&classCd=KPOP&moved=Y'

        # 차트 데이터를 파일명 "yyyy_mm_dd_mellon_chart.txt"로 저장한다.
        chartFileName = f"%4d_mellon_chart.txt" % (year)
        ...
        if req.status_code != 200:
            ...
            if not chartDatas:
                print(f'ERROR) {year}년 HTML Response에서 차트 순위를 파싱할 수 없습니다.')
                print(f'>>> {url}')
                continue
            # 파싱에 성공한 경우 차트 데이터의 개수를 보여줌
            else:
                print(f'>>> {year}년: {len(chartDatas)}건')

            fullPath = os.path.join('.', 'yearly_melon_chartdata', chartFileName)
            with open(fullPath, mode='w', encoding='utf-8') as chartFile:
                ...
                if lines:
                    ...
                    print(f'ERROR) {year}년 차트 데이터에서 곡 정보를 파싱할 수 없습니다.')
                    print(f'>>> {url}')

            print(f'{year}년 차트 데이터 파싱 완료')
            sleep(7)
